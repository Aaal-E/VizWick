...
var rootOfRoot={name:null,parent:null,children:[]};var regex=',(); ';rootOfRoot.name='rootOfRoot';var node=rootOfRoot;var subNode;for(var i=input.length-1;i>=0;i--){if(scanner==true){var n=input.charAt(i);switch(n){case')':stack.push(node);node=subNode;break;case'(':// '(' => close up the open node
node=stack.pop();break;case',':// ',' => ignore
break;case';':if(++semicolonCounter>1)scanner=false;// ';' => ignore
break;case' ':// ' ' => ignore
break;case'\n':// ' ' => ignore
break;case'\t':// ' ' => ignore
...
node.children.push(subNode);break;}}else{// => do not read stuff before the unncesseary semicolon
